...
<script setup lang="ts">
  ...
  const initFormData = async () => {
    fieldErrors.value = {}
    if (props.type === 'edit' && props.templateData) {
      const id = props.templateData.id
      try {
        const detail = await CasbinApi.template.get(id)
      ...
      if (valid) {
        ...
        try {
          if (dialogType.value === 'add') {
            await CasbinApi.template.create(formData)
            ElMessage.success('添加成功')
          } else {
            await CasbinApi.template.update(formData.id!, formData)
