...
<script setup lang="ts">
  ...
  const currentTemplateData = ref<Partial<Api.CodeGen.TemplateDto>>({})

  // 选中行
  const selectedRows = ref<Api.CodeGen.TemplateDto[]>([])
  ...
  const {
    ...
    core: {
      apiFn: (params: any) => {
        ...
        return CodeGenApi.template.getList({
  ...
  const deleteTemplate = (row: any): void => {
    ElMessageBox.confirm(`确定要删除模板 [${row.name}] 吗？`, '警告', {
      ...
      await CodeGenApi.template.del([row.id])
  ...
  const batchDelete = (): void => {
    ...
    ElMessageBox.confirm(`确定要删除选中的 ${ids.length} 个项目吗？`, '警告', {
      ...
      await CodeGenApi.template.del(ids)
      ElMessage.success('删除成功')
      refreshRemove()
    })
  }
