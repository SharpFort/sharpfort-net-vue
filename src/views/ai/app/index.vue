  ...
  <div class="p-6 h-full flex flex-col space-y-4 bg-gray-50">
    ...
    <div class="bg-white p-4 rounded shadow-sm flex-1 flex flex-col">
      ...
      >
        ...
        <el-table-column prop="type" label="应用类型" width="100" show-overflow-tooltip />
        <el-table-column prop="msgType" label="消息类型" width="100" align="center">
          <template #default="{ row }">
            <el-tag>{{ msgTypeMap[row.msgType as number] || row.msgType }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="temperature" label="温度" width="80" align="center" />
        <el-table-column prop="isAiTools" label="AI工具" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="row.isAiTools ? 'success' : 'info'">{{
              row.isAiTools ? '开' : '关'
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="isSkill" label="Skill" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="row.isSkill ? 'success' : 'info'">{{
              row.isSkill ? '开' : '关'
            }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="isHttpLog" label="日志" width="80" align="center">
          <template #default="{ row }">
            <el-tag :type="row.isHttpLog ? 'success' : 'info'">{{
              row.isHttpLog ? '开' : '关'
            }}</el-tag>
          </template>
        </el-table-column>
...
<script setup lang="ts">
  ...
  const handleDelete = async (row: Api.AiApp.AiAppDto) => {
    try {
      await aiAppManagement.delete(row.id as string)
