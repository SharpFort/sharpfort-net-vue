<template>
  <div
    class="ai-image-container h-[calc(100vh-80px)] w-full flex overflow-hidden rounded-2xl bg-slate-50 dark:bg-slate-900 !text-slate-800 dark:!text-slate-200"
  >
    <!-- Left Sidebar: Generator Control Panel -->
    <aside
      class="w-[360px] h-full bg-white/60 dark:bg-slate-800/60 backdrop-blur-2xl border-r border-slate-200 dark:border-slate-700/50 flex flex-col z-10 shadow-lg relative"
    >
      <div
        class="absolute inset-0 bg-gradient-to-b from-indigo-500/5 to-purple-500/5 pointer-events-none"
      ></div>

      <div
        class="p-6 border-b border-slate-200/60 dark:border-slate-700/60 flex items-center gap-3 relative z-10"
      >
        <div
          class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 shadow-md flex items-center justify-center text-white"
        >
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar relative z-10">
        ...
        <div class="space-y-2">
          <label
            class="flex items-center justify-between text-sm font-bold text-slate-700 dark:text-slate-300"
          >
            <span>画面描述 (Prompt)</span>
            <el-tooltip content="使用生动的形容词可以获得更好的效果"
              ><el-icon class="text-slate-400"><QuestionFilled /></el-icon
            ></el-tooltip>
          </label>
          <div
            class="relative bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 focus-within:border-indigo-500 dark:focus-within:border-indigo-500 transition-colors"
          >
            <el-input
        ...
        <div class="space-y-2">
          <label class="text-sm font-bold text-slate-700 dark:text-slate-300"
            >反向词 (Negative Prompt)</label
          >
          <el-input
            v-model.trim="form.negativePrompt"
            type="textarea"
      ...
      <!-- Generate Button -->
      <div
        class="p-6 border-t border-slate-200/60 dark:border-slate-700/60 relative z-10 bg-white/50 dark:bg-slate-900/50"
      >
        <el-button
          type="primary"
        ...
        >
          ...
          <div
            class="absolute inset-0 h-full w-0 bg-white/20 group-hover:w-full transition-all duration-500 ease-out z-0"
          ></div>
        </el-button>
        <p class="text-[11px] text-center mt-3 text-slate-400 font-medium"
          >生成过程可能需要 10~30 秒，请耐心等待</p
        >
      </div>
    </aside>
    ...
    <main class="flex-1 flex flex-col relative bg-slate-100/50 dark:bg-slate-900">
      <!-- Top Nav -->
      <header
        class="h-16 px-6 bg-white/40 dark:bg-slate-800/40 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between sticky top-0 z-20"
      >
        <div class="flex gap-6 h-full">
          <button
            @click="activeTab = 'mine'"
            :class="[
              'h-full border-b-2 font-bold px-2 transition-colors',
              activeTab === 'mine'
        ...
        <div class="flex items-center gap-2">
          <el-button
            ...
            plain
            size="small"
            class="!border-none !bg-white hover:!bg-slate-100 dark:!bg-slate-800 dark:hover:!bg-slate-700 shadow-sm"
          >
      ...
      >
        <!-- Empty State -->
        <div
          v-if="!loadingData && displayItems.length === 0"
          class="h-full flex flex-col items-center justify-center text-center opacity-60"
        >
          <el-icon :size="60" class="text-indigo-200 dark:text-indigo-900 mb-4"
            ><PictureRounded
          /></el-icon>
        ...
        <div
          v-else
          class="columns-1 md:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6 w-full max-w-[1600px] mx-auto"
        >
          <div
          ...
          >
            ...
            <div
              ...
              class="aspect-square w-full bg-slate-100 dark:bg-slate-700 flex flex-col items-center justify-center p-6 relative overflow-hidden"
            >
              <!-- Animated gradient sweeping across -->
              <div
                class="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-white/40 dark:via-white/5 to-transparent -translate-x-full animate-[shimmer_2s_infinite]"
              ></div>
              ...
              <el-progress
                ...
                class="mb-4"
              />
              <p class="text-sm font-bold text-indigo-500 animate-pulse">{{
                item.status === 0 ? '排队中...' : '渲染中...'
              }}</p>
              <p class="text-xs text-slate-400 mt-2 text-center line-clamp-2 w-full px-4">{{
                item.prompt || '暂无描述'
              }}</p>
            </div>
            ...
            <div
              ...
              class="aspect-[4/3] w-full bg-red-50 dark:bg-red-900/20 border-b border-red-100 dark:border-red-800 flex flex-col items-center justify-center p-6"
            >
            ...
            <div
              v-else
              class="relative w-full overflow-hidden bg-slate-100 dark:bg-slate-900 min-h-[200px]"
            >
              <el-image
              ...
              >
                <template #placeholder>
                  <div
                    class="w-full h-48 flex items-center justify-center text-slate-300 dark:text-slate-600"
                  >
              ...
              <!-- Hover Overlay Data -->
              <div
                class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 text-white z-10 pointer-events-none"
              >
                <p class="text-xs font-medium line-clamp-3 leading-relaxed mb-2 shadow-sm">{{
                  item.prompt
                }}</p>
                <div class="flex items-center justify-between pointer-events-auto">
                  ...
                  <div class="flex gap-2">
                    <el-button
                      ...
                      type="danger"
                      plain
                      class="!bg-black/50 !border-white/20 hover:!bg-red-500 hover:!text-white hover:!border-red-500 backdrop-blur-sm"
                    >
...
<script setup lang="ts">
  ...
  const deleteItem = async (id: string) => {
    try {
      await ElMessageBox.confirm('确定要删除此图片吗？执行后无法恢复。', '删除确认', {
        type: 'warning'
      })
...
<style scoped>
  ...
  .dark :deep(.custom-textarea-sub .el-textarea__inner) {
    background-color: rgb(15 23 42 / 40%) !important;
    border-color: rgb(51 65 85 / 80%) !important;
  }

  /* Select overriding */
  :deep(.custom-select .el-input__wrapper) {
    background-color: rgb(248 250 252 / 80%) !important;
    border: 1px solid rgb(226 232 240 / 80%) !important;
    border-radius: 8px;
    box-shadow: none !important;
  }
  ...
  .dark :deep(.custom-select .el-input__wrapper) {
    background-color: rgb(15 23 42 / 60%) !important;
    border-color: rgb(51 65 85 / 80%) !important;
  }
